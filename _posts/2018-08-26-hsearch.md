...
and dropout. In other words, we would like to generate the image as follows
(rows are size of hidden layer, columns are dropouts, values in table is
resulting accuracy):

![Hyperparameter search heatmap]({{ "/assets/images/hs.png" | absolute_url }})

Generate all combination may be quite expensive, hence it make sense to use a
cluster of machines. For distribution of computation across the cluster, we use
Rain.

## Imports and global variables

At the beginning we just import Tensorflow, Rain, and defines two global
...
the Tensorflow's example wrapped into a Rain decorated function:
...
The `@remote` decorator allows to create Rain tasks from a Python function.
...
## Running the example on CloudStack

(Now we assumes that you have an account and working configuration of ``~/.cloudstack.ini``, 
otherwise see CloudStack docs.)
